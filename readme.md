...
Not only does this solve the issue (mailtrap.io does not use a proxy service to forward images in the emails) but it also protects you from accidentally sending real emails from your test environment.

## Events

When an email is sent, viewed, or a link is clicked, its tracking information is counted in the database using the Qit\MailTracker\Models\Email model.

You may want to do additional processing on these events, so an event is fired in these cases:

- qit\MailTracker\Events\EmailEvent
...
- Qit\MailTracker\Events\PermanentBouncedMessageEvent
...
use Qit\MailTracker\Events\EmaiEvent;
...
use Qit\MailTracker\Events\PermanentBouncedMessageEvent;
